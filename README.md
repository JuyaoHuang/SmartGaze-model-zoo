...
**This is base on InsightFace. You must read the part "Notes".**
...
```
or similar/newer versions, **you MUST use the modern training scripts. Otherwise, you will encounter an overwhelming number of errors.**

Furthermore, I have provided two scripts to convert a PyTorch model to an .onnx model, and then to convert an .onnx model to a .rknn model.

However, the original code has been retained for the sake of completeness.

For specific details, please refer to the sections starting from `#### 3.4.2 Modern Training (Recommended)` and `### 3.5 Model Conversion & Deployment (RK3568)`.

> If you find this documentation too cluttered, please view the separate [README_MODERN.md](./README_MODERN.md).
> However, if you want to train either of the two models provided by the project, it is recommended to download the model parameters (model download link) provided by the original author and use them.

这个项目环境配置等**过于古老和落后**，使用原始项目会有许多报错。我在此基础上使用现代化的框架和训练方法，基本上代替了原项目的训练和评估代码。如果你的环境是:
```
torch                     2.4.1+cu118              pypi_0    pypi
torchaudio                2.4.1+cu118              pypi_0    pypi
torchvision               0.19.1+cu118             pypi_0    pypi
```
以上或差不多的版本，**请一定要使用现代的训练脚本进行训练，否则报错的条数远超您的想象**。

并且，我在此基础上提供了将 pytorch 模型转为 .onnx 模型，再将 onnx 转为 rknn 模型的两个脚本。

但为了完整性，仍然保留的原始代码。

具体部分请看：`#### 3.4.2 Modern Training (Recommended)` 以后的部分。
以及### 3.5 Model Conversion & Deployment (RK3568)。

> 如果觉得过于杂乱，跳转到另一[README_MODERN.md](./README_MODERN.md)查看。
> 但是，如果你要训练项目提供的两种模型，建议把原作者提供的模型参数（模型下载链接）下载下来使用
